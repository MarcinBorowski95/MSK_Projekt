...
//            log( "Created Federation from MSK_Fom.xml" );
    ...
    private void publishAndSubscribe() throws RTIexception {
        ...
        rtiamb.publishInteractionClass(zniecierpliwienieHandle);
    }
    ...
    private void sendZniecierpliwienieInteraction() throws RTIexception {
        ParameterHandleValueMap parameters = rtiamb.getParameterHandleValueMapFactory().create(0);
        HLAfloat64Time time = timeFactory.makeTime(fedamb.federateTime + fedamb.federateLookahead);
        liczbaKlientowHandle = rtiamb.getParameterHandle(zniecierpliwienieHandle, "kolejka");
        byte[] que = EncodingHelpers.encodeInt(queue);
        parameters.put(liczbaKlientowHandle, que);
        log("Sending liczbaKlientow");
        rtiamb.sendInteraction(zniecierpliwienieHandle, parameters, generateTag(), time);
    }
}
